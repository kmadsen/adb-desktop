...
class AdbTest {
    ...
    @Test
    fun `devices should be empty if no devices are connected`() {
        val testOutput = listOf(
    ...
    @Test
    fun `devices should be have all devices`() {
        val testOutput = listOf(
            "List of devices attached",
    ...
    @Test
    fun `wifiState detects when device id is not found`() {
        val testOutput = listOf(
    ...
    @Test
    fun `wifiState returns false if there is no dev wlan0`() {
        val testOutput = listOf(
    ...
    @Test
    fun `wifiState detects when wifi is available`() {
        val testOutput = listOf(
    ...
    @Test
    fun `wifiState detects when wifi is connected`() {
        val testOutput = listOf(
            "192.168.188.0/24 dev wlan0 proto kernel scope link src 192.168.188.150"
        )
        every { terminal.run(any()) } returns testOutput
